...
Feature: 发起模型调用
  ...
  Scenario: 非流式调用成功
    Given 账号池 "deepseek-chat" 存在且包含可用凭证
    When 客户端以访问密钥请求该账号池，且不要求流式响应
    Then 返回上游的响应且已转换为客户端协议的形态
    And 发布领域事件 UsageRecorded，包含凭证、账号池与 token 用量

  @wip @relay-invoke_model_streamed
  Scenario: 流式调用成功
    Given 账号池 "gpt-5-codex" 存在且包含可用凭证
    When 客户端以访问密钥请求该账号池，且要求流式响应
    Then 网关逐块转发上游事件直到流结束
    And 流结束后发布领域事件 UsageRecorded

  @wip @relay-invoke_model_pool_missing
  Scenario: 账号池不存在时拒绝
  ...
  Scenario: 访问密钥无效时拒绝
    Given 客户端携带无效的访问密钥
    When 客户端请求账号池 "deepseek-chat"
    Then 请求被拒绝且返回未授权
    And 不向上游发起请求

  @wip @relay-invoke_model_failover
  Scenario: 首个凭证失败时切换凭证
    Given 账号池 "deepseek-chat" 包含凭证 "openai-key-2" 与 "deepseek-main"
  ...
  Scenario: 全部凭证不可用时返回统一错误
    Given 账号池 "deepseek-chat" 的全部凭证均处于冷却状态
    When 客户端请求该账号池
  ...
  Scenario: 凭证硬配额耗尽时不再重试
    Given 账号池 "deepseek-chat" 包含凭证 "deepseek-main"
    And 该凭证返回配额耗尽且无重试时间
    When 客户端请求该账号池
    Then 请求被拒绝且凭证标记为失效
    And 不对该凭证继续重试
