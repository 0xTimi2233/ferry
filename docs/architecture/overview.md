...
空值沿层级传播：记录级 `cost`、凭证级 `month_cost` 与分档级 `UsageBucketView.cost` 都是可选字段，缺省表示金额不完整，即记录未定价、或该凭证与分档里存在未定价的记录。不计价的订阅凭证在记录级记零，因此不会使分档合计缺省；它们对 `month_cost` 仍不给出取值，因为该字段只对计价凭证有意义。
...
账号组策略为 Weighted 时取凭证自身的权重与本组内其它凭证分担流量，权重为零的凭证不参与轮询。凭证的权重与优先级在登记时给出，缺省取领域默认值，此后可由 `Credential::tune` 调整。
