...
账号组内的选择规则由 `CredentialGroup::select` 承载，冷却中与不可用的凭证一律跳过。候选集合由 `CredentialRepository::list_by_group` 取数、经 `CredentialGroup::candidates` 过滤得出，过滤只取本组凭证，结果也只能以该产物回传给挑选方法，组外凭证进不去；传给 `SelectionCursorRepository::advance` 的长度取同一份候选集合的长度，轮询位置不会与候选错位。三条策略各自的规则：轮询取决策位置指向的凭证，位置由该端口推进、聚合只消费不推进；加权按凭证权重分担流量，权重为零的凭证不参与；填满优先取优先级数值最小的凭证。权重与优先级在登记时给出，缺省取领域默认值，此后可由 `Credential::tune` 调整。
