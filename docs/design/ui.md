...
令牌沿四条正交轴线组织，改动其一不牵连其余：颜色、圆角、密度、字体。颜色令牌在 frontmatter 逐项给出十六进制值，是本设计系统的唯一真源；界面代码只引用令牌名，不直接书写色值。
...
健康状态四态对应三种标记，失效与禁用分开：

| 健康状态 | 标记 |
|---|---|
| Ready | 圆点加“可用” |
| Cooling | primary 描边胶囊，附冷却原因 |
| Disabled | 中性描边胶囊“已禁用” |
| Failed | error 描边胶囊，附失效原因 |
